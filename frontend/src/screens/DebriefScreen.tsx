...
const DebriefScreen = () => {
  ...
  const getRating = (displayIndex: number) => {
    return trustItems[displayOrder[displayIndex]];
  };

  const handleSubmit = async () => {
    if (isSubmitting) return;
    setIsSubmitting(true);
    setDebriefData({ postTrustItems: trustItems, comments });
    if (sessionCode) {
  ...
  const handleEmailSubmit = async () => {
    ...
    if (!trimmed) {
      setEmailError(language === "en" ? "Please enter an email address" : "Bitte geben Sie eine E-Mail-Adresse ein");
      return;
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(trimmed)) {
      setEmailError(language === "en" ? "Please enter a valid email address" : "Bitte geben Sie eine gültige E-Mail-Adresse ein");
      return;
    }
    setEmailError("");
            ...
            <div className="border-t border-border pt-5 mt-5 space-y-3">
              <div className="flex items-center gap-2 justify-center">
                ...
                <h3 className="text-sm font-semibold text-foreground">
                  {language === "en" ? "Want to read the paper?" : "Möchten Sie die Arbeit lesen?"}
                </h3>
              </div>
              <p className="text-xs text-muted-foreground leading-relaxed">
                {language === "en"
                  ...
                  : "Hinterlassen Sie Ihre E-Mail-Adresse und ich sende Ihnen die fertige Arbeit zu. Ihre E-Mail wird separat und anonym gespeichert - sie ist in keiner Weise mit Ihren Umfrageantworten verknüpft."}
              </p>
              {emailSent ? (
        ...
        <div className="max-w-lg mx-auto px-5 py-6 space-y-5 animate-fade-in">
          ...
          <div className="glass-panel p-4 sm:p-5 space-y-5">
            {displayOrder.map((originalIdx, displayIdx) => (
              <div key={JIAN_POST_ITEMS[originalIdx]} className="space-y-2">
                <p className="text-sm text-foreground font-medium">{t(JIAN_POST_ITEMS[originalIdx])}</p>
                  ...
                  {[1, 2, 3, 4, 5, 6, 7].map(val => (
                    <button
                      ...
                      className={`flex-1 h-8 text-xs font-medium transition-all ${
                        getRating(displayIdx) === val ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground hover:bg-accent"
                      }`}
                    >
                      {val}
                    </button>
